...
module.exports = {
  ...
  shouldAlert: function(callback) {
    AfkBot.getState(function(err, resp) {
      if (resp == true) {
        alarmLogsRef.endAt().limit(50).on("value", function(alarmLogs) {
          ...
          else { return callback(null, true) } // no motion logs, alert
        })
      }
    })
  ...
  getState: function(callback) {
    alarmArmedRef.once('value', function(alarm) {
      return callback(null, alarm.val())
    });
  }
