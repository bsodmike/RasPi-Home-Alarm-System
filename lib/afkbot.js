...
var alarmRootRef = new Firebase(process.env.FIREBASE_URL+'/alarms'),
    formattedNowTime = moment().format("dddd, MMMM Do YYYY, h:mm:ss a")
...
module.exports = {
  ...
  createAlarmLog : function(callback) {
    var alarmRootRefLogs = new Firebase(process.env.FIREBASE_URL+'/logs');
    alarmRootRefUser.set({ user : process.env.USERNAME, time : formattedNowTime, info : 'motion detected' }, function(err, success) {
      if (err) { return callback(new Error(err)) }
  ...
  alert : function(callback) {
    push.send("Motion detected: " + formattedNowTime, function(err, success) {
