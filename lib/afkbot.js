...
module.exports = {
  ...
  createAlarmLog : function(user, logInfo, callback) {
    var alarmRootRefUser = new Firebase(process.env.FIREBASE_URL+'/logs');

    alarmRootRefUser.set({ user : user, time : new Date().toISOString(), info : logInfo }, function(err, success) {
      if (err) { return callback(new Error(err)) }
      else {
    ...
    alarmRootRef.once('value', function(snap) {
      if (snap && snap.val() && snap.val().logs && snap.val().logs.time) {
        ...
        var lastDetected = moment(snap.val().logs.time)._d,
            twoMinutesAgo = moment().subtract(2, 'minutes')._d;
